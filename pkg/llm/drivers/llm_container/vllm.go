...
func (v *vllm) GetContainerSpec(ctx context.Context, llm *models.SLLM, image *models.SLLMImage, sku *models.SLLMSku, props []string, devices []computeapi.SIsolatedDevice, diskId string) *computeapi.PodContainerCreateInput {
	// Startup script to find a model and start vLLM, or wait for models to be downloaded
	startScript := `
mkdir -p ` + api.LLM_VLLM_MODELS_PATH + `
while true; do
    model=$(ls -d ` + api.LLM_VLLM_MODELS_PATH + `/* 2>/dev/null | head -n 1)
    if [ -n "$model" ]; then
        echo "Found model: $model"
        ` + api.LLM_VLLM_EXEC_PATH + ` --model $model --port ` + fmt.Sprintf("%d", api.LLM_VLLM_DEFAULT_PORT) + ` --trust-remote-code --swap-space 1
        echo "vLLM server exited with code $?. Restarting in 5 seconds..."
        sleep 5
    else
        echo "No models found in ` + api.LLM_VLLM_MODELS_PATH + `. Waiting for model to be downloaded..."
        sleep 30
    fi
done
`
	spec := computeapi.ContainerSpec{
		ContainerSpec: commonapi.ContainerSpec{
			...
			Envs: []*commonapi.ContainerKeyValue{
				...
				{
					Key:   "LD_LIBRARY_PATH",
					Value: "/lib/x86_64-linux-gnu:/usr/local/cuda/lib64:${LD_LIBRARY_PATH}",
				},
			},
		},
	}
